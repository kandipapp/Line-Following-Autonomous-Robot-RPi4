...
tmp_image_path = "/tmp/matched_symbol.jpg"
if os.path.exists(tmp_image_path):
    os.remove(tmp_image_path)
 
matched_symbol_path = "/tmp/matched_symbol.jpg"
    ...
    if len(contours) >= 2:
        ...
        if current_direction == "forward":
            ...
            if len(contours) ==2:
                ...
                if shape_name and not shape_saved:
                    cv2.drawContours(frame, [shape_contour], -1, (255, 0, 0), 2)
                   # cv2.putText(frame, shape_name, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 
                   #        1, (0, 255, 0), 2, cv2.LINE_AA)
 
                    slowForward()
                    print("Shape detected - slowing down")
 
                    # Save the frame temporarily
                    cv2.imwrite(tmp_image_path, frame)
                    print(f"Shape image saved temporarily at {tmp_image_path}")
 
                    # Optional: pause momentarily to stabilize capture
                    time.sleep(0.8)
...
def symbol_frame(frame,shape_contour):
    ...
    if best_match and max_matches >= min_match_count:
        # Draw and label the symbol
        label_text = f"Symbol: {best_match} ({max_matches})"
        cv2.putText(frame, label_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.7, (0, 255, 0), 2)
        cv2.rectangle(frame, (x_start, y_start), (x_end, y_end), (0, 255, 0), 2)
        cv2.imshow('Symbol Detection', frame)
 
        # Save frame with label if it's a special symbol (customize below)
        if "star" in best_match.lower() or "stop" in best_match.lower():
            saved_frame = frame.copy()
            cv2.imwrite(matched_symbol_path, saved_frame)
            print(f"Saved matched symbol frame: {matched_symbol_path}")
...
try:
    ...
    # At the end of the loop:
        if shape_saved and time.time() - last_shape_time > 5:
            shape_saved = False
